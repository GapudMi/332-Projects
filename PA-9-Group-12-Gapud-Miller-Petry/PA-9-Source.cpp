...
std::vector<Node> transpose(std::vector<Node> nodes, int numNodes) {
    std::vector<Node> trans;
    for (int i = 0; i < numNodes; i++) {
        Node node;
        std::string n = "N" + std::to_string(i + 1);
        node.name = n;
        node.lastTime = nodes[i].lastTime;
        trans.push_back(node);
    }
    for (Node n : nodes) {
        for (Node* child : n.edges) {
            trans[std::stoi(child->name.substr(1)) - 1].edges.push_back(&(trans[std::stoi(n.name.substr(1)) - 1]));
        }
    }
    return trans;
}
